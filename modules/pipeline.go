...
func (p *PipelineRunner) RunHeadless(ctx context.Context) error {
	...
	if err := runStep(ctx, cfg, 0, "Checking Dependencies", func() error {
	...
	// Step 1: FastQC
	if err := runStep(ctx, cfg, 1, "Quality Control with FastQC", func() error {
	...
	// Step 2: fastp
	if err := runStep(ctx, cfg, 2, "Quality Trimming with fastp", func() error {
	...
	if cfg.EnableSortMeRNA {
		if err := runStep(ctx, cfg, 3, "rRNA Filtering with SortMeRNA", func() error {
	...
	if err := runStep(ctx, cfg, 4, stepName, func() error {
	...
	// Step 5: CPC2
	if err := runStep(ctx, cfg, 5, "Coding Potential with CPC2", func() error {
	...
	if err := runStep(ctx, cfg, 6, "Cross-Validation with CPAT", func() error {
	...
	if err := runStep(ctx, cfg, 7, "Processing GTF and Filtering lncRNAs", func() error {
	...
	// Step 8: RNAfold
	if err := runStep(ctx, cfg, 8, "lncRNA Secondary Structure Prediction", func() error {
	...
	if cfg.EnableLncTar {
		if err := runStep(ctx, cfg, 9, "Predicting lncRNA-mRNA Interactions with LncTar", func() error {
	...
	if cfg.EnableIntaRNA {
		if err := runStep(ctx, cfg, 10, "Cross-Validating Targets with IntaRNA", func() error {
	...
	// Step 11: Consensus
	if err := runStep(ctx, cfg, 11, "Cross-Tool Consensus Analysis", func() error {
	...
	// Step 12: Enrichment
	if err := runStep(ctx, cfg, 12, "Building Enrichment Gene Lists", func() error {
	...
	// Step 13: RSeQC
	if err := runStep(ctx, cfg, 13, "RNA-seq Quality Assessment with RSeQC", func() error {
	...
	// Step 14: IGV Report
	if err := runStep(ctx, cfg, 14, "Creating IGV Genome Browser Report", func() error {
	...
	// Step 15: MultiQC
	if err := runStep(ctx, cfg, 15, "Generating MultiQC Report", func() error {
	...
	if err := runStep(ctx, cfg, 16, "Generating Pipeline Summary Report", func() error {
...
// It respects --resume and --from-step by calling ShouldSkipStep first.
func runStep(ctx context.Context, cfg *config.Config, stepNum int, name string, fn func() error) error {
	if ShouldSkipStep(cfg, stepNum) {
		return nil
	}
